...
function CartContent() {
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-2xl mx-auto py-20 px-4 text-center">
        <div className="bg-white rounded-[3rem] p-12 warm-shadow border border-[#5A5A40]/5">
          ...
          <h2 className="text-3xl font-serif text-[#5A5A40] mb-4">Seu carrinho está vazio</h2>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <h1 className="text-4xl font-serif text-[#5A5A40] mb-12">Meu Carrinho</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-6">
          <AnimatePresence>
            {cart.map((item) => (
              <motion.div
                ...
                exit={{ opacity: 0, x: -20 }}
                className="bg-white rounded-3xl p-6 warm-shadow border border-[#5A5A40]/5 flex flex-col sm:flex-row items-center gap-6"
              >
                ...
                <div className="flex-grow text-center sm:text-left">
                  <h3 className="text-xl font-serif text-[#4a4a40] mb-1">{item.name}</h3>
                  <p className="text-sm text-[#4a4a40]/50 uppercase tracking-wider mb-2">{item.category}</p>
                  <p className="text-[#C5A059] font-medium">
                    R$ {item.price.toFixed(2).replace('.', ',')}
                  </p>
                </div>
                
                <div className="flex items-center bg-[#f5f5f0] rounded-full p-1 border border-[#5A5A40]/10">
                  <button
                  ...
                  <span className="w-10 text-center font-medium text-[#5A5A40]">{item.quantity}</span>
                  <button
                    onClick={() => updateQuantity(item.id, item.quantity + 1)}
                    className="p-2 hover:bg-white rounded-full transition-colors text-[#5A5A40]"
                  >
          ...
          <button 
            onClick={() => clearCart()}
            className="text-[#4a4a40]/50 hover:text-[#ff4444] text-sm font-medium transition-colors flex items-center ml-auto"
          >
            Limpar Carrinho
          </button>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 warm-shadow border border-[#5A5A40]/5 sticky top-32">
            ...
            <form onSubmit={handleFinalize} className="space-y-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-wider text-[#5A5A40] mb-2 ml-1">
                    Nome Completo
                  </label>
                  <div className="relative">
                    <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#5A5A40]/40" />
                    <input
                      type="text"
                      required
                      value={name}
                      ...
                      className="w-full bg-[#f5f5f0] border border-[#5A5A40]/10 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-[#C5A059]/20 transition-all"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#5A5A40]/40" />
                    <input
                      type="tel"
                      required
                      value={phone}
              ...
              <div className="border-t border-[#5A5A40]/10 pt-6 space-y-4">
                <div className="flex justify-between text-[#4a4a40]/70">
                  <span>Subtotal ({totalItems} itens)</span>
                  <span>R$ {totalPrice.toFixed(2).replace('.', ',')}</span>
                </div>
                <div className="flex justify-between items-end">
                  <span className="text-lg font-serif text-[#5A5A40]">Total</span>
                  <span className="text-3xl font-serif text-[#5A5A40] font-bold">
              ...
              <button
                type="submit"
                className="w-full bg-[#25D366] text-white py-5 rounded-full font-bold text-lg flex items-center justify-center space-x-3 hover:opacity-90 transition-all shadow-lg active:scale-95"
              >
            ...
            <p className="mt-6 text-center text-xs text-[#4a4a40]/50 leading-relaxed">
